...
def fix_file(filename="chinese_char_gifs.txt"):
    ...
    for char in CHARS_TO_FIX:
        ...
        if gif_url:
            # 确保是有效的 bcebos 链接
            if 'bcebos.com' in gif_url and '.gif' in gif_url and 'icons' not in gif_url:
                results[char] = gif_url
    ...
    # 保存结果 - 按汉字顺序排序
    print("\n保存修复结果...")
    sorted_results = dict(sorted(results.items(), key=lambda x: list(results.keys()).index(x[0])))
